...
task main()
	var n : int64 = 4 -- grid size along each dimension
		...
		for i = [int](tiles.bounds.lo.x), [int](tiles.bounds.hi.x) + 1 do
			for j = [int](tiles.bounds.lo.y), [int](tiles.bounds.hi.y) + 1 do
				...
				if i > tiles.bounds.lo.x and j > tiles.bounds.lo.y then
					average_upper_corner(tile_partition[{i,j}], tile_partition[{i-1,j}],tile_partition[{i,j-1}],tile_partition[{i-1,j-1}])
				end
				if i > tiles.bounds.lo.x then 
					average_left_col(tile_partition[{i,j}], tile_partition[{i-1,j}])
				end 
				if j > tiles.bounds.lo.y then
					average_upper_row(tile_partition[{i,j}], tile_partition[{i,j-1}])
				end
